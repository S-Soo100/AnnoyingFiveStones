...
2026-08-22 게임 | 작업: 시안 0822 A덩어리 — 홈 4번째 "납골당" 버튼 + 문구 5건 (Standard, 4파일). (1) TitleScreenUI 버튼 스택 y 679/789/899 → **640/750/860/970**(시안 Buttons 380×410 @770,600), 4번째 버튼 신설 — v18에 "시안이 3개로 확정"이라며 지웠던 그 입구를 시안 0822가 되살림. (2) GraveyardUI.ShowViewOnly() 신설(내 기록 미추가 관람 모드 — 기존 isTestPlay 플래그 재사용, 호출부 가독성용 래퍼). (3) LocalizationManager: home.cemetery(묘지/Cemetery) → home.columbarium(납골당/Columbarium), grave.record_title EN "Save Record"→"Record", grave.name_prompt "묘비"→"유골함"·EN→"Enter a name for the urns", grave.save EN 마침표 제거, grave.elapsed EN "Time taken"→"Time", pause.resume EN "Restart"→**"Resume"(의도적 시안 이탈, 디자이너 재확정)**. (4) NameInputUI characterLimit 16→15(시안 주석 "닉네임 15자 제한"). | 참조: general#1,#2,#4 game#13 | 지킴: general#1(시안 수치를 기억·스크린샷이 아니라 talk-to-figma 실측으로 — Buttons 프레임 bbox와 텍스트 노드 fontSize/좌표를 노드별로 확인), general#2(최소 변경 — Settings/Pause/Dialog는 시안과 이미 일치해 손대지 않음, y값·문구·상수만 교체), general#4(**자체 검수에서 2건 적발** — ①타이틀 캔버스 250 > 납골당 200이라 타이틀을 안 내리면 화면이 그대로다 → Hide(콜백) 경유로 수정, ②버튼 4개가 되며 스택 아래변이 시안 y=1010까지 내려와 하단 토스트 띠(시안 960~1035)가 "납골당" 버튼을 덮음 → 토스트를 로고·버튼 사이 빈칸(510~585)으로 이동), game#13(수정 전 TitleScreenUI/GraveyardUI/NameInputUI/GameManager.RestartGame을 직접 Read해 진입·복귀 경로를 끝까지 추적). | 놓침: 없음(컴파일 0 error, 신규 warning 0 — 잔존 12건은 기존 obsolete TMP/미사용 필드). | 재발: 없음. | 메모: **"시안이 확정했다"는 주석은 유통기한이 있다** — v18에 "시안이 Play/Settings/Exit 3개로 확정"이라 적고 지운 버튼이 4개월 뒤 시안 0822에서 부활했다. 다행히 그 주석이 되돌리는 법(PlayerPrefs "EndingSeen")까지 남겨둬서 복구가 5분이었다. **지울 때 되돌리는 법을 함께 적는 습관이 실제로 값을 했다.** 반대로 pause.resume은 "디자이너 확정"이라 적어둔 게 이번에 뒤집혔으므로 주석에 재확정 날짜를 박아 다음 전수 대조 때 또 뒤집히지 않게 했다(3864c03에서 의도적 예외 5건이 시안대로 되돌아간 전례). 토스트 충돌은 **시안에 없는 우리 자산이 시안 변경에 치인 사례** — 다음 시안 반영 때도 "시안에 없는 요소들(토스트·디버그HUD·커튼)이 새 레이아웃과 겹치지 않는가"를 체크리스트로 돌릴 것.
  ↳ (이어서) B덩어리 — 기록 저장 창 수치 + ✕ 이탈 경고 (Standard, 2파일) — (1) NameInputUI 몸통 배치를 0822 실측으로: Slide x70→**60**·w540→**560**, 안내문 y145→**137**, 소요시간 y284→**292**, 저장버튼 y375→**383**(입력칸 212는 불변). (2) ✕ 동작 변경 — 기존 "✕=저장"(주석: 이 창은 취소가 성립하지 않는다)에서 **주의 창 경유**로. `discard.*` 키 4종 신설 + `BuildDiscardPanel`(Dialog 680×485, 문구 592×102@44,80, 버튼 284×80@44/352,242 — 일시정지 종료창과 동일 규격). [아니오]=기록저장 창 복귀(입력 포커스 재획득), [게임 종료]=미저장 + `RestartGame(true)` 타이틀 복귀. | 참조: general#1,#2,#4 game#13 | 지킴: general#4(**자체 검수 3건** — ①`onNameConfirmed`를 안 부르면 GameManager 엔딩 코루틴이 `WaitUntil(nameConfirmed)`에 영구 정지하는데, 그 코루틴이 L771에서 `transitionCoroutine`에 담기고 `RestartGame`이 이를 StopCoroutine+isTransitioning=false로 정리함을 **코드로 확인 후** 진행. ②주의 창 위에서 입력이 계속 먹히지 않도록 `DeactivateInputField`. ③영문 "Exit the game without saving your progress?"를 자동 줄바꿈에 맡기면 나눔고딕이 Iosevka보다 넓어 3줄로 넘칠 위험 → 시안의 실제 줄바꿈 지점(모노스페이스 592/24≈24자 → "without" 뒤)을 계산해 수동 개행), general#1(문구를 기억으로 옮기지 않고 593:441·593:456·593:426을 각각 스캔 — 그 결과 **`quit.*`가 국·영문 혼종**이었음을 발견: 국문은 "기록을 저장하지 않고…"(0822가 discard 창에 배정), 영문은 "Quit game? Your score won't be"(0822가 인게임 종료창에 유지). 0822에 두 창이 **나란히 살아 있으므로** 별개 창으로 분리하고 `quit.*`는 손대지 않음). | 놓침: 없음(0 error, 신규 warning 0). | 재발: 없음. | 메모: **"이 창은 취소가 성립하지 않는다"는 단정이 기획 변경으로 뒤집혔다** — A덩어리의 "시안이 3개로 확정"과 같은 패턴의 2회째. 코드 주석의 단정형 서술("~할 길이 없다", "~로 확정")은 그 시점의 기획이지 불변식이 아니다. **승격 후보**: "주석에 기획 단정을 적을 땐 근거 시안 버전·날짜를 함께 적는다"(3회째 시 정식 룰화). C덩어리(납골당 재디자인)는 **배경 아트 부재로 착수 불가** — 시안이 벽을 1920×988 사진 타일 2장으로 깔고 그 위에 284×288 유골함 카드를 얹는 구조인데, 카드 행 피치(326.94)와 타일 높이(988)가 정수배가 아니라(3행=980.82, 타일당 7.18 어긋남) **3행부터의 실제 니치 위치는 배경 이미지를 재봐야 안다**. 시안은 1·2행만 배치해 두어 근거가 없음. 좌표 추측 금지(game#21) → 아트 확보 후 진행.
  ↳ (이어서) C덩어리 — 납골당 화면 사진합성 재설계 (Critical, 1파일 전면개편 + 에셋 1종) — 흰 배경 + 회색 카드 3열 GridLayoutGroup → **벽 사진 타일(1920×988) 반복 + 시안 좌표 절대배치**. `LayoutWall(n)`이 타일을 세로로 이어 content 높이를 정하고, `CreateNiche(i,…)`가 카드 284×288을 ColX{485.94,810,1134.06} × RowY{23.06,350,676.94}에 놓는다(타일당 9칸). 버튼은 우하단 가로 2개 → 시안 593:712의 **우측 세로 2개**(1528, 798/908, 300×80 — Go Home 291.5는 300으로 통일)로 옮기고 UIWindow.MakeButton 공통 부품으로 교체. 내 기록은 맨 뒤 덧붙이기 → **순위 자리에 삽입**(`InsertMyRecord`, 서버 정렬키 regression asc·time asc 그대로 + GET/POST 경주 대비 중복 제거). 시안대로 내 기록 강조·회귀 표시 없음. | 참조: general#1,#2,#4 game#19,#21 images#9 | 지킴: **game#21(좌표 추측 금지)을 이번엔 제대로 지켰다** — 시안 좌표를 그대로 믿지 않고 `wall_tile.png`의 칸막이를 행/열 밝기 프로파일로 실측(니치 안쪽 y 17~312 / 348~642 / 677~972, 기둥 사이 x 480~782 / 806~1109 / 1131~1421)한 뒤, **PIL로 벽 사진 위에 카드·텍스트 사각형을 합성해 눈으로 대조**하고 진행. general#4(자체 검수에서 **로딩 중 빈 화면 회귀** 적발 — 벽을 content 안으로 옮기면서 전체화면 흰 배경을 지웠더니 Supabase 대기(최대 15초) 동안 게임 화면이 그대로 비쳤다. `CoLoadAndScroll` 진입 즉시 `LayoutWall(0)`), general#1(UIWindow.MakeButton이 HandCursorHoverTrigger를 이미 붙이는지 **코드로 확인** 후 기존 조립 코드 삭제 — 손커서 반응이 조용히 사라지는 걸 막음), images#9(원본 `figma-export/260822figma/Back.png`를 덮어쓰지 않고 4px 페더 여백을 잘라 `Assets/Resources/Columbarium/wall_tile.png`로 별도 저장). | 놓침: **유골함 카드(284×288) 에셋이 내보내기에 빠져 미완** — 코드는 `Columbarium/urn_card`를 읽고 없으면 경고 후 글자만 얹는다. 화면은 뜨지만 시안과 다르다. | 재발: 없음. | 메모: **행 피치(326.94)와 타일 높이(988)가 정수배가 아니다**(3행=980.82). 행을 계속 더하면 타일마다 7.18px씩 밀려 결국 칸막이를 밟으므로 **타일마다 RowY를 처음부터 다시 쓴다**. 실측 니치 안쪽에 세 행이 모두 들어가는 것을 확인했다(3행은 676.94 vs 니치 677 — 0.06 차이로 사실상 일치). **Resources/는 빌드에 통째로 들어가므로** 이번에 쓰는 wall_tile 한 장만 임포트하고 Back-1(영정)·Back-2(국화)는 배선할 때 옮긴다. Dim70/Dim85는 순수 검정 균일 알파(178/217)라 **텍스처가 필요 없다** — `Color(0,0,0,0.698f/0.851f)`로 충분(픽셀 검사로 확인). 벽 사진이 스크롤과 함께 움직이는 구조라 **버튼만 캔버스 직속**으로 남겨야 한다 — content에 붙이면 스크롤에 딸려 올라간다.
  ↳ (이어서) D덩어리 — 엔딩 영정 배경 + 문구 규격 (Standard, 1파일 + 에셋 1종) — `GameUI.ShowLifeEnd`를 시안 0822(593:724)에 맞춰 **88px→76px Bold, 중심 y 562→570**, 배경 검정 → **영정 사진**(국화+검은 액자). `DoFullscreenLine`에 `bold`/`background` 선택 인자를 더해 크레딧 농담(593:728, 40px Regular)의 기존 동작은 그대로 뒀다. | 참조: general#1,#2,#4 game#21 images#9 | 지킴: game#21(**Dim을 겹칠지 말지를 추측하지 않고 픽셀로 판정** — 시안 텍스트 상자 800×194 @(560,473)이 액자 검정면 안에 들어가는 비율을 계산해 100%임을 확인 → Dim 불필요 결론. 액자 안쪽 실측 x 479~1450 / y 240~848. 이어서 PIL로 영정 위에 76px 나눔고딕을 합성해 눈으로 대조), general#4(**자체 검수에서 하드컷 회귀 적발** — 배경이 검정일 때는 다음 연출(크레딧)도 검정이라 `overlayGroup.alpha=0` 스냅이 안 보였는데, 사진을 깔면 글자만 사라지고 영정이 그대로 있다가 뚝 끊긴다. 사진일 때만 0.6초 페이드 추가. **검정→검정이라 가려져 있던 결함이 배경이 생기자 드러난 사례**), general#1(overlayBg가 다른 연출에서 `color=(0,0,0,a)`로 재사용되는 것을 grep으로 전수 확인 → 사진이 남으면 검게 물드므로 `StopOverlay`·`RestoreOverlayMainLayout` 두 리셋 지점 모두에서 sprite를 null로), general#2(공유 코루틴 `DoFullscreenLine`을 갈아엎지 않고 기본값 인자 2개만 추가 — 크레딧 경로는 한 글자도 안 바뀜). | 놓침: **크레딧 배경(책상 사진 2장)은 내보내기에 없어 검정 유지.** 문구 규격(40px, y=941)은 이미 일치해 코드 변경 없음. | 재발: 없음. | 메모: **월드스페이스 캔버스가 2500×1400이라 전체 스트레치 배경은 화면(24.889 world)보다 0.45% 넓다** — 사진이 좌우 4px씩 잘리고 0.45% 늘어난다. 육안 식별 불가라 그대로 뒀지만, 정밀이 필요한 배경이 또 들어오면 bg rect를 2489로 좁혀야 한다. `Dim70`/`Dim85`는 이번에도 안 썼다(순수 검정 균일 알파 — 텍스처 불필요). **남은 미보유 에셋 2종**: 유골함 카드 284×288(`593:696`) · 크레딧 책상 배경(`593:728`).
  ↳ (이어서) 테스트 피드백 3건 수정 (Standard, 2파일 + 에셋 1종) — (1) **유골함 카드 누락** → `player-pot.png`(292×296, 4px 페더)를 284×288로 잘라 `Resources/Columbarium/urn_card.png`. 코드는 이미 이 경로를 읽고 있어 임포트만으로 해결. 벽+카드+텍스트를 PIL로 합성해 시안과 대조 확인. (2) **관람 모드에 Play Again** → `isViewOnly` 플래그 + `ApplyEndButtonMode()`. 홈에서 들어오면 Play Again을 감추고 Go Home만 **시안 첫 번째 자리(y=798)**로 올린다(아래 칸에 혼자 두면 위가 비어 어색). (3) **전환 시 날것의 씬 노출** → BootCurtain 적용. 들어갈 때: 타이틀(250) 뒤에 커튼(200)을 즉시 세우고 타이틀을 페이드아웃 → 드러나는 건 검정. 나갈 때: 커튼 0.25초 올린 뒤 RestartGame. | 참조: general#1,#3,#4 game#13 | 지킴: general#3(**진단 먼저** — 증상만 보고 페이드를 늘리지 않고, BootCurtain 주석("타이틀→게임 전환 시 노출되는 미초기화 화면을 가리는 막")과 호출처 grep으로 *이 경로만 커튼이 빠져 있다*는 걸 확정. 유저 스크린샷의 하늘 그라디언트+회색 탁자가 정확히 그 "미초기화 화면"이었다), general#4(자체 검수에서 **상태 리셋 누락** 적발 — `isViewOnly`를 켜고 나가면 다음 ALL CLEAR 때 Play Again이 계속 숨겨진다. 진입 경로 3곳(Show/ShowViewOnly/ShowPreview) 모두에서 모드를 다시 적용), general#1(커튼과 납골당이 **둘 다 sortingOrder 200**이라 누가 위인지 정해지지 않는 것을 확인 → 납골당을 190으로 내려 커튼 아래로 확정. 위쪽 이웃 LifePanorama 210·타이틀 250·설정 260·이름입력 300은 불변, 아래 이웃 GameUI 100·Tutorial 120도 불변임을 전수 확인). | 놓침: 없음(0 error, 신규 warning 0). | 재발: **있음 — "Overlay는 sortingOrder와 무관하게 WorldSpace 위"를 또 밟았다.** StoryMentUI가 v10에 같은 이유로 WorldSpace→Overlay 220으로 옮긴 전례가 있고(그 주석에 규칙이 적혀 있다), 이번엔 커튼(Overlay 200) vs 납골당(Overlay 200) 동률 문제로 나타났다. **승격 후보 2회째**: "새 전체화면 UI를 만들 때 렌더모드+sortingOrder를 기존 표와 대조한다". | 메모: **별건 발견 — 첫 플레이 튜토리얼이 커튼에 가려질 가능성.** `TitleScreenUI.DoStartBehindCurtain`이 커튼(Overlay 200, blocksRaycasts=true)을 올린 채 `StartGameFromTitle`을 부르고, `TutorialSeen==0`이면 `TutorialUI`(**WorldSpace** 120)를 띄운다. Overlay는 WorldSpace 위이므로 튜토리얼이 검은 막 뒤에 갇히고 입력도 막힌다. 커튼을 내리는 `StartStage`의 `FadeOut`은 튜토리얼 콜백 **이후**라 스스로 못 풀린다. 코드 흐름상 첫 실행 하드락 — 실기 확인 필요. 고치려면 StoryMentUI와 같은 처방(Overlay 220) 또는 튜토리얼 표시 전 FadeOut. 이번 작업 범위 밖이라 손대지 않고 보고만 함.
  ↳ (이어서) 엔딩 시퀀스 3건 — 크레딧 배경 · 전환 암막 · 기록저장 배경 (Standard, 3파일 + 에셋 1종) — (1) **시안의 "Credit" 프레임(593:728)이 역할 목록이 아니라 농담 문구 화면**이었다. 배경(국화 593:729 + Dim85 + 인생사진 2장 593:731/732)이 붙을 자리는 DoCredit이 아니라 **ShowEndingJoke**. 사진 2장은 새 에셋이 아니라 게임이 쓰던 `Life/age10`(교실)·`age25`(책상) — 시안 렌더와 대조표를 눈으로 맞춰 확정. 국화는 미사용으로 남겨뒀던 `Back-2.png`. (2) **엔딩 전 구간 암막** — `GameUI.BeginEndingBlackout()/EndEndingBlackout()` 신설. 주마등 직전부터 납골당이 뜬 다음 프레임까지 검은 막을 유지한다. 오버레이를 검정막/사진/암막/인생사진/글자 5층으로 쪼개, 사진을 걷을 때 **검정 위로** 걷히게 했다. (3) **기록 저장 창 배경** — 시안 Lanking 593:645~647 그대로 납골당 벽 타일 2장 + Dim70(0.698). | 참조: general#1,#2,#3,#4 game#21 | 지킴: general#3(증상만 보고 페이드를 늘리지 않고 **새는 지점을 전수 열거** — ①HideOverlay→주마등 ②주마등→영정 ③영정 페이드아웃 ④영정→크레딧 ⑤크레딧→농담 ⑥농담→이름입력 ⑦이름입력→납골당 1프레임. 각각을 어떤 층이 덮는지 캔버스 렌더모드/sortingOrder로 확인 — GameUI는 WorldSpace 100이라 이름입력(Overlay 300)·납골당(Overlay 190) **아래**에 깔려서 시퀀스 전체를 덮을 수 있다는 게 이 설계의 근거), general#4(**자체 검수에서 CanvasRenderer.SetAlpha 오용 적발** — 카드가 흰 테두리(부모)+사진(자식) 2단인데 SetAlpha는 자식에 전파되지 않아 테두리만 사라진다. CanvasGroup으로 교체), game#21(기울기를 추측하지 않고 2.35°/5°/8° 후보를 로컬 합성해 시안 렌더와 A/B → **5°** 확정. absoluteBoundingBox 역산(2.35°)은 시안 렌더와 눈에 띄게 달라 기각), general#2(공유 코루틴 DoFullscreenLine에 선택 인자 2개(dimAlpha·lifePhotos)만 추가, 역할 크레딧 화면은 시안에 없으므로 검정 그대로). | 놓침: 없음(0 error, 신규 warning 0). | 재발: **있음 — "연출 끝 = 오버레이 투명"이 2회째.** D덩어리에서 영정 하드컷을 고치며 overlayGroup 전체를 페이드했는데, 그게 곧 "막까지 투명"이라 게임이 비쳤다. 한 번 고친 자리에서 같은 종류의 결함을 다시 만든 것. **승격 후보 2회째**: "전체화면 연출을 끝낼 때 *무엇이 드러나는지*를 반드시 명시한다(다음 연출/검정/게임)". | 메모: **"Credit"이라는 레이어 이름을 우리 코드의 DoCredit과 같은 것으로 읽은 게 1차 오독**이었다. 시안 프레임 이름과 코드 함수 이름이 같아도 같은 화면이라는 보장이 없다 — 프레임 **안의 텍스트**로 대조해야 한다. Dim70/Dim85는 이번에도 텍스처를 안 썼다(순검정 균일 알파 178/217 → Color 알파 0.698/0.851). 남은 미보유 에셋 0종 — 시안 0822의 모든 화면이 자산을 갖췄다.
- **2026-08-25 | 플레이테스트 피드백 0825 6건 (Standard, 6파일)** — (1) 받기 허용을 보드 앞변까지 확장: HandController 하늘 경계를 catch phase && !isHolding일 때 FrontScreenY로 + 낙하 종료 시 최후 판정 추가, CatchSystem 낙 판정선도 앞변으로(단, boardSurfaceY=SkyFloorY는 5단이 이어받으므로 별도 CatchFailY로 분리). (2) 35살 장애물: OnScatterComplete의 자 밀어내기가 transform 직접 이동이라 v17 BoardPos가 낡아 "안 집힘" + 다른 돌 위 텔레포트로 물리 사출 "밀림" → SetBoardMotion + 겹침 회피 + 보드 클램프. (3) 손 커짐: ApplyCatchVisualScale이 접힌 손가락으로 손끝 거리를 재면 배율이 커짐 → 측정 전 즉시 펼침. (4) 나이: 누적이 아니라 "루프 시작 나이(StageConfig.Age SOT) + 클리어 단 수" 파생으로 — 누적이면 실패 재도전 시 이중 가산으로 60살 조기 클리어. 가이드 조건 age==10 → loop==1 교체. (5) 줍기: IsStoneUnderHand를 BoardPos → transform 역투영으로(OnBoard 돌은 비-kinematic이라 밀린 뒤 BoardPos가 낡음). (6) 원근: BackHalfWidth 4.66→6.24(비율 0.583→0.78, 돌 크기차 1.71→1.28배) — 테스트가 "물체 축소율=보드 폭 축소율"을 잠가서 비율 자체를 조정. | 참조: general#1,#2,#3,#4 game#13,#18,#19,#21 | 지킴: general#1(SkyFloorY/BoardSurfaceY/UnifiedQuad 소비처 전수 grep 후 5단 경로 보존 확인, BoardGeometryTests가 원근 분리를 금지함을 확인하고 우회 대신 SOT 상수 조정), game#18(SOT 변경 다운스트림 전수 검토 — CurrentAge 소비처 5곳/BackHalfWidth 소비처 2곳 확인). | 놓침: 없음(0 error, EditMode 15/15). | 재발: 없음. | 메모: **v14 교훈 "안착 돌 isKinematic=true"가 v17에서 비-kinematic(damping)으로 바뀌어 있었다** — 이번엔 판정을 transform 기반으로 바꿔 우회했지만, 밀림 자체(시각)는 남는다. 다음 밀림 보고 시 OnBoard kinematic화(+기믹 영향 검토)가 근본 처방.
